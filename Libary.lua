...
function Library:CreateWindow(opts)
    ...
    if typeof(opts.Parent) == "Instance" then
        ...
        local preferPlayerGui = (opts.Mobile == false)
        if typeof(getgenv) == "function" then
            local g = getgenv()
            if g.OxideUsePlayerGui == true or g.OxideForcePC == true then
                preferPlayerGui = true
            end
        end
        if preferPlayerGui then
            pcall(function()
                local lp = Players.LocalPlayer or Players.LocalPlayer
                if lp then targetParent = lp:WaitForChild("PlayerGui", 12) end
            end)
        end
