...
local function resolveTravelSpeed(requested, inSafeZone)
    ...
    if kickSafeMode then
        local cap = inSafeZone and KICK_SAFE_ZONE_SPEED or KICK_SAFE_MAX_SPEED
        return math.clamp(requested, 50, cap)
    end
...
local function MoveToPoint(target, speed, easeOut)
    ...
    speed = resolveTravelSpeed(speed, target.X < SAFE_BOUNDARY_X)
...
local function FlyToPoint(target, speed, easeOut)
    ...
    -- CFrame-only glide: never set AssemblyLinearVelocity (client AC flags fake velocity).
    speed = resolveTravelSpeed(speed, target.X < SAFE_BOUNDARY_X)
    local t0 = os.clock()
    local totalDist = dist
    while not HUB.dead do
        ...
        if easeOut then
            local progress = 1 - math.clamp(remain / totalDist, 0, 1)
            stepSpeed = math.max(speed * (1 - progress * 0.85), 32)
        end
        local step = math.min(stepSpeed * dt, remain)
        local dir = toTarget.Unit
        local nextPos = curPos + dir * step
        ...
        hrp.AssemblyLinearVelocity = Vector3.zero
        hrp.AssemblyAngularVelocity = Vector3.zero
        if os.clock() - t0 > (totalDist / 45 + 8) then break end
    end
...
local function getSafeZoneSpeed()
    return kickSafeMode and KICK_SAFE_ZONE_SPEED or 245
end
...
local function TravelRoadPath(targetPos, speed, isApproach)
    ...
    if isReturningToBase and startPos.X > SAFE_BOUNDARY_X then
        ...
        MoveToPoint(pBaseRoad, getSafeZoneSpeed(), false)

        -- 4. Enter base pen at safe-zone speed
        local pPen = targetPos + Vector3.new(0, 1.2, 0)
        MoveToPoint(pPen, getSafeZoneSpeed(), isApproach == true)
        return true
    else
        local p1 = Vector3.new(startPos.X, safeY, MAIN_ROAD_Z)
...
local function TravelFlyDirect(targetPos, speed, isApproach)
    ...
    if isReturningToBase and startPos.X > SAFE_BOUNDARY_X then
        ...
        FlyToPoint(pGroundSafe, getSafeZoneSpeed(), false)
...
-- Yumurta alindiktan sonra base'e donus: kosu bandina (treadmill) dusmemek icin
-- ana yol cizgisine (MAIN_ROAD_Z) uc, tam hizalan, sonra dikey in.
local LINE_Z_TOLERANCE = 1.35

local function TravelReturnLineDrop(safePlotCenter, speed)
    local hrp = findHRP()
    if not hrp or not safePlotCenter then return false end
    if avoidTrapsEnabled then pcall(NeutralizeTraps) end

    speed = resolveTravelSpeed(speed, false)
    local penY = math.max(safePlotCenter.Y, 70.4)
    local flyBoost = kickSafeMode and 22 or 32
    local flyAlt = math.max(hrp.Position.Y, penY, 70.4) + flyBoost
    local destX = safePlotCenter.X
    local startPos = hrp.Position

    if startPos.Y < flyAlt - 6 then
        FlyToPoint(Vector3.new(startPos.X, flyAlt, startPos.Z), speed, false)
        startPos = hrp.Position
    end

    FlyToPoint(Vector3.new(startPos.X, flyAlt, MAIN_ROAD_Z), speed, false)

    if startPos.X > SAFE_BOUNDARY_X then
        FlyToPoint(Vector3.new(SAFE_BOUNDARY_X, flyAlt, MAIN_ROAD_Z), speed, false)
    end

    FlyToPoint(Vector3.new(destX, flyAlt, MAIN_ROAD_Z), math.min(speed, getSafeZoneSpeed() + 40), false)

    local pos = hrp.Position
    if math.abs(pos.Z - MAIN_ROAD_Z) > LINE_Z_TOLERANCE then
        FlyToPoint(Vector3.new(pos.X, flyAlt, MAIN_ROAD_Z), getSafeZoneSpeed(), false)
    end

    local dropTarget = Vector3.new(destX, penY + 1.2, MAIN_ROAD_Z)
    FlyToPoint(dropTarget, getSafeZoneSpeed(), true)

    local pPen = safePlotCenter + Vector3.new(0, 1.2, 0)
    if (hrp.Position - pPen).Magnitude > 3.5 then
        FlyToPoint(pPen, getSafeZoneSpeed(), true)
    end
...
StealSub:AddToggle({
    Name = "Kick-Safe Mode (slower, anti-detect)", Default = kickSafeMode, Flag = "kick_safe",
    Callback = function(v)
        kickSafeMode = v
...
StealSub:AddSlider({
    Name = "Glide / Travel Speed", Min = 50, Max = kickSafeMode and 280 or 750,
    Default = glideSpeed, Suffix = " studs/s", Flag = "glide_speed",
    Callback = function(v)
        glideSpeed = resolveTravelSpeed(v, false)
    end
})
StealSub:AddSlider({
    Name = "Steal Delay Gap", Min = 0.5, Max = 10, Default = stealDelay, Suffix = "s", Flag = "steal_gap",
